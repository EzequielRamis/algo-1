	...
	(\bloque{+2em}{
		\paraTodo{h}{hogar}{
			h\in th \implica (\bloque{+2em}{
				h[@hogA\tilde n o] = head(th)[@hogA\tilde no] \wedge\\
...
\pred{todosHogaresConMasHabitacionesQueDormitorios}{th: $eph_h$}{
	\neg \existe{h}{hogar}{\#habitaciones(h) < h[@ii2]}
}
...
\begin{proc}{histHabitacional}{\In th: $eph_h$, \In ti: $eph_i$, \In region: \ent, \Out res: \TLista{\ent}}{}
	...
	\post{ longitudDeHistograma(th, res) \yLuego
		\bloque{+6em}{
			(\forall{i: \ent})
			(0\leq i < |res| \implicaLuego res[i] =	\#casaConNHabitaciones(th, i + 1))
		}
	}
\end{proc}

\aux{\#casaConNHabitaciones}{th: $eph_h$, n: \ent}{\ent}{\sum_{h\in th}\IfThenElse{esCasa(h) \wedge \#habitaciones(h) = n}{1}{0}}

\pred{longitudDeHistograma}{th: $eph_h$, res: \TLista{\ent}}{
	\existe{h}{hogar}{
		h\in th \wedge\\ \paraTodo{g}{hogar}{
			g\in th \implica \#habitaciones(h)\geq \#habitaciones(g)
...
\begin{proc}{laCasaEstaQuedandoChica}{\In th: $eph_h$, \In ti: $eph_i$, \Out res: \TLista{\float}}{}
	...
	\post{ |res| = 6 \yLuego
		\bloque{+6em}{
			\paraTodo{i}{\ent}{
				0 \leq i < |res| \implicaLuego\\ res[i] =
				totalCasasCriticasPorRegion(th, ti, i + 1) / totalCasasPorRegion(th, i + 1)
			}
		}
	}
\end{proc}
